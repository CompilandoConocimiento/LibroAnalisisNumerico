...
\clearpage
    ...
    \chapter{Interpolante de Newton}
        ...
        % ==============================================================
        \clearpage
        \section{Coeficinetes: Diferencias Divididas}
        ...
        % ==============================================================
        \clearpage
        \section{División Homogenea}

            Si es que podemos asegurar de que nuestros puntos estan tomados de una partición
            homogenea (de distance $h$) entonces se pueden simplificar muchas cosas.

            Pues se puede hacer el cambio de variable tal que:
            $x_s = x_0 + sh$ para tantas $s$ como elementos en una partición.
            Ahora, siendo incluso mas general podemos decir que  $x = x_0 + sh$ $\forall s \in \Reals$

            Nota que con esto ahora:
            $x - x_i = (s - i)h$

            Y con esto la podemos expresar el interpolante como:
            \begin{MultiLineEquation*}{3}
                I(x) := I(x_0 + sh)  
                    &= \sum_{k = 0}^n s(s-1)\dots(s-k+1)h^k f[x_0, \dots, x_k]  \\
                    &= f[x_0] + \sum_{k = 1}^n { s \choose k } k! h^k f[x_0, \dots, x_k]  \\
            \end{MultiLineEquation*}
    ...
    % ===============================================================================
    \chapter{Interpolante de Hermite}
        ...
        \section{Definición}
            ...
            La idea es generar a partir de un grupo de $n$ puntos, un interpolantes, 
            pero el truco es que dicho interpolante también concuerde en sus devivadas,
            es decir, asegurarme que el interpolante y la función original tengan la misma
            pendiente.

            Por eso no solo necesitamos un grupo de puntos, sino también del valor númerico
            de las derivadas en esos puntos.

            Por eso también será un polinomio de grado $2n+1$.

        % ==============================================================
        % =================        INTERPOLANTE       ==================
        % ==============================================================
        \section{Interpolante}
        ...
        \section{Propiedades}
            ...
            El polinomio de Langrange cumple que:
            \begin{MultiLineEquation*}{3}
            L_{n, j}(x_i) = 
                \begin{cases}
                    0 & \text{ si } j \neq i    \\
