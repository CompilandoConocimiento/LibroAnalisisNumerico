...
% =======================================================
    ...
    \newcommand \ColorFunction       {Teal700MD}                    %Color definition
    \newcommand \ColorRoot           {Red700MD}                     %Color definition

    \newcommand \Function[1]     {\Color{\ColorFunction}{f\Wrap{#1}}}   %Use: f(x) = y
    \newcommand \Root         {\Color{\ColorRoot}{\xi}}                 %Use: f(root) = 0
...
\clearpage
    ...
    \chapter{Tolerancias}
        ...
        \section{Ideas}
            ...
            \begin{itemize}
                \item $\Function{\Root_k} < \epsilon$: Estamos muy cerca de que la función valga cero
    ...
    \chapter{Bisección}
        ...
        \section{Algoritmo}
            ...
            Suponte que tienes una linda función $\Function{x} = 0$
            y tienes dos números $a, b$ tal que $\Function{a}$ sea de un signo diferente
            al $\Function{b}$ entonces estamos seguros de que existe una raíz $\Root$
            por ahí en medio.

            Entonces, puedes tomar el punto intermedio de ambos 
            $c = a + (b - a) / 2$. Ahora pueden pasar 3 cosas:
            \begin{itemize}
                \item Si $c$ es la raíz entonces ya estas
                \item Si $[a, c]$ tienen signos diferentes entonces la raíz anda por ahí
                \item Si $[c, b]$ tienen signos diferentes entonces la raíz anda por ahí
            \end{itemize}
            
            En general, basta con suponer que el signo de $a$ es igual que el signo de $c$,
            de ser así, entonces $c$ cumple el trabajo de $a$ entonces el intervalo es $[c, b]$.
            De no ser así entonces tiene que hacer el trabajo de $b$ y entonces el intervalo es
            $[a, c]$.
    ...
    \chapter{Punto Fijo}
        ...
        \section{Ideas}
            ...
            Sea una función $g(x)$ entonces decimos que $\Root$ es un punto fijo si 
            es que $\Function{\Root} = \Root$.

            Suponte que podemos escribir a $\Function{x} = 0$ como 
            $\Function{x} = g(x) - x = 0$.

            Entonces cualquier punto fijo $\Root$ será una raíz de $\Function{x}$.

            La idea es entonces bastante intuitiva, toma un elemento inicial $x_n$ entonces
            $x_{n+1} = g(x_n)$ será una mejor aproximación a la raíz.
            ...
            \subsection{Propiedades}
                ...
                \begin{itemize}
                    \item Sea $f(x) = 0$ escrita de forma $f(x) = g(x) - x = 0$, es decir
                        $g(x) = x$.

                        Entonces si $\forall x \in [a, b]$ se cumple que:
                        \begin{itemize}
                            \item $g(x) \in [a, b]$
                            \item $g(x)$ toma todos los valores entre $a$ y $b$
                            \item $g(x)'$ existe en $(a, b)$ y existe una constante $0 < r < 1$ tal que
                                $\Abs{g'(x)} \leq r$
                        \end{itemize}

                        Va a pasar que: 
                        \begin{itemize}
                            \item Existe un único punto fijo $x = \Root$ de g(x) entre $[a, b]$
                            \item La sequencia $x_{k+1} = g(x_k)$ converge siempre a $\Root$
                        \end{itemize}
